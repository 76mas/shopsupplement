...
const ProductDetailsContent = ({
  ...
  const selectedFlavor = controlledFlavor !== undefined ? controlledFlavor : internalFlavor;
  ...
  const displayPrice = product.endPrice
    ? Number(product.endPrice)
    : product.discountPrice ?? Number(product.price);

  const originalPrice =
    product.price && Number(product.price) !== displayPrice
          ...
          <AnimatePresence mode="wait">
            {images.length > 0 ? (
              ...
              <div className="w-full h-full flex items-center justify-center text-gray-300">
                <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="3" y="3" width="18" height="18" rx="2" /><circle cx="9" cy="9" r="2" /><path d="m21 15-5-5L5 21" /></svg>
              </div>
            )}
          </AnimatePresence>
          {product.discount && (
        ...
        <div className="flex items-center gap-4 mb-8">
          <div className="flex flex-col">
            <span className="text-3xl lg:text-4xl font-black text-black">
              {displayPrice.toLocaleString()} <span className="text-lg">د.ع</span>
            </span>
            {originalPrice && (
        ...
        {flavors.length > 0 && (
          <div className="mb-10">
            <div className="flex justify-between items-center mb-5">
              ...
              <span className="text-sm lg:text-base font-black text-black">{flavors[selectedFlavor]?.name}</span>
            </div>
        ...
        {sizes.length > 0 && (
          <div className="mb-12">
            <div className="flex justify-between items-center mb-5">
              ...
              <span className="text-sm lg:text-base font-black text-black">{sizes[selectedSize]?.name}</span>
            </div>
            <div className="flex flex-wrap gap-3">
              {sizes.map((size, idx) => (
                <button
                ...
                >
                  ...
                  {size.price && size.price !== Number(product.endPrice ?? product.price) && (
          ...
          {product.isAvailable ? (
            <>
              <div className="flex items-center justify-between bg-[#F5F5F5] rounded-[20px] px-6 py-4 w-44 border border-gray-200/50">
                <button onClick={() => setQuantity(Math.max(1, (quantity || 1) - 1))} className="text-2xl font-bold hover:scale-125 transition-transform cursor-pointer">-</button>
                <span className="font-black text-xl">{quantity || 1}</span>
                <button onClick={() => setQuantity((quantity || 1) + 1)} className="text-2xl font-bold hover:scale-125 transition-transform cursor-pointer">+</button>
              </div>
