  ...
  return (
    <>
      <div className="flex justify-between border-b border-[#88888833] pb-4 mb-4">
        <p className="font-bold text-black text-lg">تصنيف حسب</p> <FilterIcon />
      </div>

      <div className="flex flex-col gap-3">
        {[
          { name: "الكل", count: 10, active: true },
          { name: "بروتينات", count: 4, active: false },
          { name: "حوارق دهون", count: 3, active: false },
          { name: "طاقة وباور", count: 2, active: false },
          { name: "فيتامينات", count: 1, active: false },
        ].map((cat, index) => (
          <div
            key={index}
          ...
          >
            <div className="flex items-center gap-3">
              <div
                className={`w-5 h-5 border border-[#88888855] rounded-[6px] group-hover:border-black transition-all flex items-center justify-center ${cat.active ? "border-black" : ""}`}
              >
                <div
                  className={`w-2.5 h-2.5 bg-black rounded-[2px] opacity-0 group-hover:opacity-20 transition-opacity ${cat.active ? "opacity-100" : ""}`}
                ></div>
              </div>
              <span className="text-black group-hover:text-black transition-colors font-medium">
                {cat.name}
              </span>
            </div>
      ...
      <div className="mt-8">
        ...
        <div className="relative w-full h-2 bg-[#F1F1F1] rounded-full mb-8">
          <div
            ...
            style={{
              left: `${((150000 - maxPrice) / (150000 - 0)) * 100}%`,
              right: `${((minPrice - 0) / (150000 - 0)) * 100}%`,
            }}
          ></div>
        ...
        <div className="flex flex-col gap-3">
          <div className="flex justify-between items-center text-sm font-medium">
            ...
            <div className="w-4 h-[1px] bg-[#ddd] mt-4"></div>
          ...
          <button className="w-full mt-4 cursor-pointer bg-black text-white py-4 rounded-[20px] font-bold text-sm hover:bg-black/80 transition-all active:scale-[0.98]">
            تطبيق الفلتر
          </button>
        </div>
      </div>
